...
from utils.utils import check_config_file_state, zip_files, convert_repo_config_csv_to_csv

if "form-submitted" not in st.session_state:
    st.session_state["form-submitted"] = False
...
with st.sidebar:
    ...
    config_type = st.radio(
        "Select type of configuration file to create",
        ["repo-config.csv", "author-config.csv", "group-config.csv", "report-config.json", "config.json"]
    )
...
match config_type:
    case "repo-config.csv":
        ...
        for count in range(num_repo):
            branch_count[count] = st.number_input(
                label=f"Select the number of branches to include in branch {count + 1}",
                key=f"repo-config-branch-count{count}",
                min_value=1,
                step=1
            )

        st.write("#### Configuration Entries")
        with st.form("repo-config-form"):
            form_returns: dict[str, dict[str, dict]] = {}

            for branch_number, branches in branch_count.items():
                base_key = str(branch_number)  # defines the base key for all widgets to avoid key errors

                st.markdown(f"## Repository {branch_number + 1}")
                ...
                for branch in range(branches):
                    ...
                    branch_base_key = str(branch)  # defines the base key for all widgets in this branch
                    ...
                    find_prev_authors = st.checkbox(
                        ...
                        value=False,
                        help="If enabled, RepoSense will utilize Git blame's ignore revisions functionality to blame "
                             "the line changes caused by commits in the ignore commit list to the previous authors "
                             "who altered those lines (if available)")
                    ...
                    shallow_cloning = st.checkbox(
                        ...
                        help="If enabled, RepoSense will utilize Git' shallow clone functionality. This option may "
                             "reduce the time taken to clone repositories, but should be disabled for smaller `.git` "
                             "files of size < 500 MB due to overhead incurred")
                    ...
                    skip_ignored_file_analysis = st.checkbox(
                        ...
                        value=False,
                        help="If enabled, RepoSense will ignore analysis of files exceeding the file size entirely. "
                             "If skipped, all information about the file will be omitted from the report [can "
                             "possibly improve report generation time!]")
                    ...
                    ignore_glob_lists = st_tags(
                        label="Enter in the list of file path globs to ignore during analysis for each author (Refer "
                              "to [this](https://docs.oracle.com/javase/tutorial/essential/io/fileOps.html#glob) for "
                              "more info on path glob syntax)",
                        text="e.g. test/**, temp/**...",
                    ...
                    ignore_authors_list = st_tags(
                        label="Enter in the list of authors to ignore during analysis, specified by [Git Author Name]"
                              "(https://reposense.org/ug/configFiles.html#a-note-about-git-author-name)",
            ...
            if st.form_submit_button("Create RepoSense Configuration!"):
                ...
                st.session_state["form-submitted"] = is_valid
    ...
    case "author-config.csv":
        st.subheader("`author-config.csv` Configuration Wizard")
        st.write("Select the number of repositories to configure for, and how many branches per repository!")

        num_repo = st.number_input("Select the number of repositories to include", min_value=1)
        branch_count = {}

        for count in range(num_repo):
            branch_count[count] = st.number_input(f"Select the number of branches to include in branch {count + 1}",
                                                  min_value=1, step=1)

        with st.form("author-config-form"):
            for branch_number, branches in branch_count.items():
                st.write(branch_number, branches)

            st.form_submit_button("Create RepoSense Configuration!")
    case "group-config.csv":
        ...
        with st.form("group-config-form"):
            for branch_number, branches in branch_count.items():
                st.write(branch_number, branches)
    ...
    case "report-config.json":
        ...
        with st.form("report-config-form"):
            for branch_number, branches in branch_count.items():
                st.write(branch_number, branches)
    ...
    case "config.json":
        ...
        with st.form("config-form"):
            ...
            st.form_submit_button("Create RepoSense Configuration!")
    case _:
        st.info("Select a type of configuration file to create!")
